...
else 
{
	...
	// if the patrol car does not exist, redirecr back to update.php
...
while ($row = $result->fetch_assoc()) {
$patrolCarStatusArray['patrolar_status_id'] = $row['patrolcar_status_desc'];
  }
...
if (isset($_POST["btnUpdate"])){
	...
	$conn = new mysqli(DB_SERVER, DB-USER, DB_PASSWORD, DB_DATABASE);
	// Check connction
	if ($conn->connect_error) {
		die("Connection failed: "  . $conn->connect_error);
	}
...
/* if patrol car status is Arrived (4) then capture thhe time of arrival */
...
/* else if patrol car status is FREEE (3) then capture the time of completion */ else if ($_POST["patrolCarStatus"] == '3') {
